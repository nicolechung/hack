...
public class GBFlee : MonoBehaviour {
	...
	bool hasObstacles(Vector2 direction, string colorString) {
		...
		if (DEBUG) Debug.DrawRay (transform.position, direction*directionDistance, color);
		...
		hitsLeft =  Physics2D.RaycastAll (leftOrigin, directionLeft, directionDistance, 1 << LayerToMask);
			if (DEBUG) Debug.DrawRay (leftOrigin, directionLeft*directionDistance, color);
		...
		hitsRight =  Physics2D.RaycastAll (rightOrigin, directionRight, directionDistance, 1 << LayerToMask);
			if (DEBUG) Debug.DrawRay (rightOrigin, directionRight*directionDistance, color);
